...
{ 
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            // sprawdzenie czy konto usera jest potwierdzone
            var user = UserManager.FindByName(model.Email);
            if (user != null && !user.EmailConfirmed)
            {
                ModelState.AddModelError("", "Konto nie jest potwierdzone");
                return View(model);
            }
        ...
        public async Task<ActionResult> RegisterStudent(RegisterStudentViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await UserManager.SendEmailAsync(user.Id, "Potwierdz konto", "    <a href=\"" + callbackUrl + "\">here</a>");
